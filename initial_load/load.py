...
try:
    market = pandas.read_csv('/data/DEINFO_AB_FEIRASLIVRES_2014.csv')
except:
    print("File DEINFO_AB_FEIRASLIVRES_2014.csv not found. It should be present in current folder")
    sys.exit(1)

market.columns = ['id', 'longitude', 'latitude','setcens','areap','cod_dist','district','cod_sub_prefecture','sub_prefecture','region_5','region_8','name','registration_code','address_street','address_number','address_city','reference']

#ID is autogenerated
market = market.drop(columns=['id'])

market.to_sql('markets_market', engine, if_exists='append', index = False, method=None)

print("Initial load successfully done")